"""
ORVIS Voice Server — Servidor de síntese de voz otimizado.
"""
import os
import shutil
import uuid
...
async def gerar_audio(req: GenerateRequest):
    ...
    try:
        ...
        # Executa a geração em thread separada para NÃO travar a API (Async)
        def run_tts():
            tts.tts_to_file(
                text=texto_limpo,
                speaker_wav=amostras_voz,
        ...
        # Pós-processamento pesado apenas se solicitado (Modo HQ)
        if req.high_quality:
